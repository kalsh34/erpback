...
export class RotationService {
  ...
  static computeDayAssignments(rot: IRotation, date: Date): { guardId: mongoose.Types.ObjectId; shiftType: 'DAY' | 'NIGHT'; shiftTime: string }[] {
    ...
    // Slot plan: all DAY slots first, then all NIGHT slots, padded with REST
    // entries until the plan length equals the pool size. This is the full
    // circular slot ring every guard rotates through.
    const slotPlan: ('DAY' | 'NIGHT' | 'REST')[] = [];
    for (let i = 0; i < rot.dayShiftCount; i += 1) slotPlan.push('DAY');
    for (let i = 0; i < rot.nightShiftCount; i += 1) slotPlan.push('NIGHT');
    while (slotPlan.length < poolSize) slotPlan.push('REST');

    // Each day the ring advances by exactly ONE position, so every guard
    // walks the whole ring over a poolSize-day cycle and experiences Day,
    // Night, and Rest (nobody is permanently typecast into one shift type).
    const startDate = new Date(rot.startDate);
    startDate.setHours(0, 0, 0, 0);
    const dayOffset = Math.max(0, Math.floor((date.getTime() - startDate.getTime()) / 86400000));
    const offset = dayOffset % poolSize;

    const assignments: { guardId: mongoose.Types.ObjectId; shiftType: 'DAY' | 'NIGHT'; shiftTime: string }[] = [];
    for (let i = 0; i < poolSize; i += 1) {
      const slotType = slotPlan[(i + offset) % poolSize];
      if (slotType === 'REST') continue;
      const guard = activeGuards[i];
  ...
  static async preview(id: string, days: number) {
    ...
    const guardIds = [...new Set(result.map((r) => r.guardId.toString()))];
    const Employee = mongoose.model('Employee');
    const guardDocs = await Employee.find({ _id: { $in: guardIds } }).select('firstName lastName employeeCode');
    const guardMap = new Map(guardDocs.map((g: any) => [g._id.toString(), g]));

    const populatedAssignments = result.map((r) => ({
      ...r,
      guard: guardMap.get(r.guardId.toString()) || null,
    }));

    const guardStats = guardIds.map((gid) => {
      const gd = guardMap.get(gid);
      const guardAssignments = populatedAssignments.filter((r) => r.guardId.toString() === gid);
      const dayShifts = guardAssignments.filter((r) => r.shiftType === 'DAY').length;
      const nightShifts = guardAssignments.filter((r) => r.shiftType === 'NIGHT').length;
      return {
        guardId: gid,
        name: gd ? `${gd.firstName} ${gd.lastName}` : 'Unknown',
        employeeCode: gd?.employeeCode || '',
        dayShifts,
        nightShifts,
        totalShifts: dayShifts + nightShifts,
        restDays: days - dayShifts - nightShifts,
      };
    });
