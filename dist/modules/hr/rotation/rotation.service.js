...
class RotationService {
    static computeDayAssignments(rot, date) {
        ...
        const slotPlan = [];
        for (let i = 0; i < rot.dayShiftCount; i += 1)
            slotPlan.push('DAY');
        for (let i = 0; i < rot.nightShiftCount; i += 1)
            slotPlan.push('NIGHT');
        while (slotPlan.length < poolSize)
            slotPlan.push('REST');
    ...
    static async preview(id, days) {
        ...
        const Employee = mongoose_1.default.model('Employee');
        const guardDocs = await Employee.find({ _id: { $in: guardIds } }).select('firstName lastName employeeCode');
        const guardMap = new Map(guardDocs.map((g) => [g._id.toString(), g]));
