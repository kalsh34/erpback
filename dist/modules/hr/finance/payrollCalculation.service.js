 ...
 */
async function pensionBaseForGuard(grossPay, workedSalary, asOf = new Date()) {
    ...
    if (!rule)
        return workedSalary;
    return rule.pensionTaxBase === types_1.PensionTaxBase.GROSS_PAY ? grossPay : workedSalary;
}
class PayrollCalculationService {
    ...
    static async calculateGuardPayroll(recordId) {
        ...
        const { PayrollPeriod: GuardCalcPeriod } = await Promise.resolve().then(() => __importStar(require('../../../models/PayrollPeriod')));
        const guardPeriodId = record.payrollPeriodId?._id || record.payrollPeriodId;
        ...
        const pension = await this.calculatePension(await pensionBaseForGuard(grossPay, workedSalary, guardAsOf), guardAsOf);
        const incomeTax = await this.calculateIncomeTax(grossPay, guardAsOf);
        const baseComponent = pension.pensionTaxBase === types_1.PensionTaxBase.GROSS_PAY
            ? grossPay
            : workedSalary;
    ...
    static async calculateStaffPayroll(recordId) {
        ...
        // Resolve time-sensitive config against the payroll period, not wall-clock time.
        const { PayrollPeriod: StaffCalcPeriod } = await Promise.resolve().then(() => __importStar(require('../../../models/PayrollPeriod')));
    ...
    static async generateGuardPayrollRecords(payrollPeriodId) {
        ...
        const assignmentByGuard = new Map();
        ...
        for (const guard of guards) {
            ...
            if (existing)
                continue;
            const assignment = assignmentByGuard.get(guard._id.toString());
