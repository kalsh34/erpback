...
exports.slotWindow = slotWindow;
exports.requiredRestMs = requiredRestMs;
...
function slotWindow(cfg, date, shiftType) {
    if (shiftType === 'FULL') {
...
function requiredRestMs(cfg, window) {
    if (cfg.shiftMode === 'SINGLE_24H')
        return 48 * HOUR_MS;
...
function restBefore(windows, window) {
    let latestEnd = -1;
...
function restAfter(windows, window) {
...
function hasOverlap(guard, window) {
    for (const w of guard.committed)
        if (overlaps(w, window))
            return true;
    for (const w of guard.external)
        if (overlaps(w, window))
            return true;
    return false;
}
...
function restLabel(restMs) {
    if (!Number.isFinite(restMs))
        return 'no previous duty';
...
function betterEligible(a, b) {
    if (a.state.totalShifts !== b.state.totalShifts)
        return a.state.totalShifts < b.state.totalShifts ? a : b;
    const ar = Math.min(a.restBeforeMs, MAX_REST_CAP_MS);
    const br = Math.min(b.restBeforeMs, MAX_REST_CAP_MS);
    if (ar !== br)
        return ar > br ? a : b;
    if (a.typeMismatch !== b.typeMismatch)
        return a.typeMismatch < b.typeMismatch ? a : b;
    if (a.balancePenalty !== b.balancePenalty)
        return a.balancePenalty < b.balancePenalty ? a : b;
    if (a.state.order !== b.state.order)
        return a.state.order < b.state.order ? a : b;
...
function buildSchedule(cfg) {
    ...
    for (let d = 0; d < Math.max(1, cfg.days); d += 1) {
        ...
        const slotTypes = [];
        for (let i = 0; i < daySlots; i += 1)
            slotTypes.push(single24 ? 'FULL' : 'DAY');
        for (let i = 0; i < nightSlots; i += 1)
            slotTypes.push('NIGHT');
        for (const shiftType of slotTypes) {
            ...
            for (const st of states.values()) {
                ...
                const typeMismatch = shiftType === 'DAY' ? (st.lastShiftType === 'NIGHT' ? 0 : 1)
                    : shiftType === 'NIGHT' ? (st.lastShiftType === 'DAY' ? 0 : 1)
                        : 0;
                const balancePenalty = shiftType === 'NIGHT' ? st.nightShifts - st.dayShifts : st.dayShifts - st.nightShifts;
                const cand = { state: st, restBeforeMs: rb, restAfterMs: ra, typeMismatch, balancePenalty };
                const compliant = rb >= requiredRest && ra >= requiredRest;
                if (compliant && (!bestEligible || betterEligible(cand, bestEligible)))
                    bestEligible = cand;
                if (!bestOverride || betterOverride(cand, bestOverride))
                    bestOverride = cand;
            }
...
//# sourceMappingURL=shiftSchedule.algorithm.js.map